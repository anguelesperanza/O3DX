...
mkdir %ODINASM%
echo.
echo === Building Triangle-Example (Citro3D Phase 5) ===
echo.
:: -------------------------------
:: Compile vertex shader with picasso
:: Outputs romfs/vshader.shbin which is loaded at runtime via fopen.
:: -------------------------------
echo Compiling vertex shader...
picasso -o "%ROMFS%\vshader.shbin" "vshader.v.pica"
if errorlevel 1 goto :fail
echo  - vshader.v.pica -^> romfs\vshader.shbin
:: -------------------------------
...
:: (Triangle-Example only needs ctru_bridge and c3d_bridge — no c2d_bridge)
:: -------------------------------
echo.
...
:: -------------------------------
:: Link ELF
:: Note: no -lcitro2d — this example uses citro3d directly.
:: -------------------------------
echo.
echo Linking ELF...
...
smdhtool --create "Triangle-Example" "Triangle-Example" "Anguel" ^
    "%DEVKITPRO%\libctru\default_icon.png" "Triangle-Example.smdh"
if errorlevel 1 goto :fail
:: -------------------------------
:: Convert ELF → 3DSX
:: -------------------------------
echo.
echo Creating 3DSX...
3dsxtool "app.elf" "app.3dsx" --smdh="Triangle-Example.smdh" --romfs="%ROMFS%"
echo.
